...
<table cellpadding="0" cellspacing="1" border="0" class="align_center list_table" id="admin_table">
            	<thead>
                   	<tr>
                        ...
                        <th width="190" order="up">被顶次数</th>
                   		<th width="250" order="down">被踩次数</th>    
                    </tr>
                </thead>
                <tbody>
                
          		  	<?php if (!empty($_obj['ratings'])){if (!is_array($_obj['ratings']))$_obj['ratings']=array(array('ratings'=>$_obj['ratings'])); $_stack[$_stack_cnt++]=$_obj; $rowcounter = 0; foreach ($_obj['ratings'] as $rowcnt=>$ratings) { $ratings['ROWCNT']=($rowcounter); $ratings['ALTROW']=$rowcounter%2; $ratings['ROWBIT']=$rowcounter%2; $rowcounter++;$_obj=&$ratings; ?>
            		<tr>
               		 	<td><?php echo $_obj['person_name']; ?></td>
               		    <td><?php echo intval($_obj['up']);?></td>
						<td><?php echo intval($_obj['down']);?></td>
               		</tr>
...
$(function()
{
	...
	$('[order]').each(function()
	{
		...
		$(this).html('');
		//新的排序
		var new_url = url.replace(/\&(order|desc|asc)=\w+/ig,'');
		new_url+= '&order='+order;
