<div >
            <table cellpadding="0" cellspacing="0" border="0" id="item_table" class="list_table" >
            	<thead>
                   	<tr>
                		<th width="" order="original_filename">文件名</th>
						<th width="">预览视频</th>
                ...
                <tbody>
            		...
            		<tr>
               		 	<td><?php echo $_obj['original_filename']; ?></td>
               		 	<td><a href="/player/player.php?flvID=<?php echo $_obj['video_file_id']; ?>" target="_blank">预览</a></td>
                        <td><?php echo $_obj['category_name']; ?></td>
               		    <td><?php echo preg_replace("/\:\d\d$/","",$_obj['created_at']);?></td>
						<td><div admin_type="text"
		              	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=status&id=<?php echo $_obj['video_file_id']; ?>" 
...
?></div></td>
                        	<td>
	                            ...
	                            <a href="javascript:;" onclick="if (confirm(' 确定要删除吗？')) change_status(<?php echo $_obj['video_file_id']; ?>,2,this);"> 删除</a>
								<?php endif;?>&nbsp;
