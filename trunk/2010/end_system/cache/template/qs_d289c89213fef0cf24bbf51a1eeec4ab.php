<div >
            <table cellpadding="0" cellspacing="0" border="0" id="item_table" class="list_table" >
            	<thead>
                   	<tr>
                		<th width="50"> 优先级</th>
                		<th width="" order="title">标题</th>
                        <th width="100" order="category_id"> 所属栏目</th>
                       <th width="130" order="created_at">发布时间</th>
                ...
                <tbody>
            		...
            		<tr>
                        	...
                        	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=order_id&id=<?php echo $_obj['program_id']; ?>" 
                            admin_trigger="click" style="text-align:center"><?php echo $_obj['order_id']; ?></div>
                        </td>
               		 	<td><div admin_type="text"
                        	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=title&id=<?php echo $_obj['program_id']; ?>" 
                            admin_trigger="click"><?php echo $_obj['title']; ?></div>
                        </td>
                        <td><div admin_type="text"
                        	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=category_id&id=<?php echo $_obj['program_id']; ?>" 
						...
						<td><div admin_type="text"
		              	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=status&id=<?php echo $_obj['program_id']; ?>" 
...
?></div></td>
                        	<td>
	                        	...
	                        	<a href = "admin.php?p=item&action=edit_item&item_id=<?php echo $_obj['program_id']; ?>&category_id=<?php echo $_stack[0]['category_id']; ?>" > 编辑</a>
								<?php endif;?>&nbsp;
								<?php if (check_show('delete')):?>
	                            <a href="javascript:;" onclick="if (confirm(' 确定要删除吗？')) change_status(<?php echo $_obj['program_id']; ?>,2,this);"> 删除</a>
								<?php endif;?>&nbsp;
								<a href="admin.php?p=comment&itemid=<?php echo $_obj['program_id']; ?>&itemtype=program" target="_blank">评论</a> &nbsp;
								<a href="admin.php?p=programannounce&program_id=<?php echo $_obj['program_id']; ?>" target="_blank">公告</a>			
	                        </td>
               		</tr>
