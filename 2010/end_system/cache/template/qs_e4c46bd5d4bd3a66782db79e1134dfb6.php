            ...
            <table cellpadding="0" cellspacing="0" border="0" id="item_table" class="list_table" >
                ...
                <tbody>
            		...
            		<tr>
                        	...
                        	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=order_id&id=<?php echo $_obj['video_id']; ?>" 
                            admin_trigger="click" style="text-align:center"><?php echo $_obj['order_id']; ?></div>
                        </td>
               		 	<td><div admin_type="text"
                        	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=title&id=<?php echo $_obj['video_id']; ?>" 
                            admin_trigger="click"><?php echo $_obj['title']; ?></div>
                        </td>
                        <td><div admin_type="text"
                        	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=category_id&id=<?php echo $_obj['video_id']; ?>" 
						...
						<td><div admin_type="text"
		              	admin_para="m=update&table=<?php echo $_stack[0]['table']; ?>&column=status&id=<?php echo $_obj['video_id']; ?>" 
...
?></div></td>
                        	<td>
	                        	...
	                        	<a href = "admin.php?p=item&action=edit_item&item_id=<?php echo $_obj['video_id']; ?>&category_id=<?php echo $_stack[0]['category_id']; ?>" > 编辑</a>
								<?php endif;?>&nbsp;
								<?php if (check_show('delete')):?>
	                            <a href="javascript:;" onclick="if (confirm(' 确定要删除吗？')) change_status(<?php echo $_obj['video_id']; ?>,2,this);"> 删除</a>
								<?php endif;?>&nbsp;
								<a href="admin.php?p=comment&itemid=<?php echo $_obj['program_id']; ?>&itemtype=video" target="_blank">评论</a>			
	                        </td>
               		</tr>
