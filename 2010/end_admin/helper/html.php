...
function endcms_input_file($data)
{
	...
	echo "<input type='file' name='".$data['field_name']."' style='";
...
function endcms_input_select($data)
{
	...
	{
		echo "<option ";
		if ($content[$name] == $_val) echo " selected='selected' ";
...
function endcms_input_datetime($data)
{
	...
	$_style = $data['style'];
	echo "<input type='text' name='$name' style='$_style' value='".$data['content']."' />";
	if (!$data['description']) $data['description'] = lang('eg. 2010-11-23 12:23:59');
	echo $data['description'];
}
...
function endcms_input_richtext($data)
{
	...
	echo htmlspecialchars($data['content']);
	echo "</textarea>";
	echo "</td></tr><tr><Td>{UPLOAD_ATTACHMENT}</td><td>";
	echo '<iframe src="admin.php?p=upload&for=<?php echo $name;?>" style="width:700px;height:30px;border:0;text-align:left;" border="0" frameborder="no" scrolling="no"></iframe>';
}